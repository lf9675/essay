            ...
            with st.spinner("AI 正在仔细批改你的作文，请稍候约30秒……"):
                try:
                    ...
                    with st.spinner("AI 正在批改你的作文，请稍候约40秒……"):
                        # v5.0：max_tokens 16000 → 24000（v5 输出含 topic_analysis / why_change /
                        # how_to_change / 三层 revised / 双语 voice，比 v4.9 多 ~50% 长度。
                        # 16000 在 12 段长作文场景会截断 paragraph_feedback,导致逐段批改不显示）
                        full_text_chunks = []
                        stop_reason = None
...
# ═══════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════
    # v5.0：完整提升路径卡片已删除（用户反馈：太学术化、太罗嗦）
    # 学生看的关键路径已经在『老师总评』+『最重要的建议』里了，不再单独展示
    # ══════════════════════════════════════════════════════════
