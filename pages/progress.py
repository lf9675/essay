...
st.set_page_config(page_title="学生进步追踪", page_icon="📈", layout="wide", initial_sidebar_state="collapsed")
...
with nav1:
    if st.button("🏠 首页", key="nav_home_p"):
        st.switch_page("app.py")
with nav2:
    if st.button("🎓 学生作文提交", key="nav_student_p"):
        st.switch_page("pages/student.py")
with nav3:
    if st.button("👩‍🏫 教师管理后台", key="nav_admin_p"):
        st.switch_page("pages/admin.py")
with nav4:
    st.button("📈 学生进步追踪", key="nav_progress_p", disabled=True)
